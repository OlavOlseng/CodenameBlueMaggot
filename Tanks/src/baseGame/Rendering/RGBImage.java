...
public class RGBImage {
	...
	public void DrawCircle(int color,int x0,int y0, int radius){
		...
		int f = 1 - radius;
		  ...
		  setPixel(pixels,color,x0, y0 + radius);
		  setPixel(pixels,color,x0, y0 - radius);
		  setPixel(pixels,color,x0 + radius, y0);
		  setPixel(pixels,color,x0 - radius, y0);
		  ...
		  while(x < y)
		  {
		    ...
		    ddF_x += 2;
		    f += ddF_x;    
		    setPixel(pixels,color,x0 + x, y0 + y);
		    setPixel(pixels,color,x0 - x, y0 + y);
		    setPixel(pixels,color,x0 + x, y0 - y);
		    setPixel(pixels,color,x0 - x, y0 - y);
		    setPixel(pixels,color,x0 + y, y0 + x);
		    setPixel(pixels,color,x0 - y, y0 + x);
		    setPixel(pixels,color,x0 + y, y0 - x);
		    setPixel(pixels,color,x0 - y, y0 - x);
		  }
	...
	public void setPixel(int[] pixels,int color,int x, int y){
